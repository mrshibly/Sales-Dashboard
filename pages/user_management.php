...
// Only HOM can access this page


// Fetch all users for reports_to dropdown
$user = new User($db);
$all_users_query = "SELECT id, name, role FROM users";
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="reports_to" name="reports_to">
                            ...
                            <?php foreach ($all_users as $u): ?>
                                <option value="<?php echo $u['id']; ?>" data-role="<?php echo $u['role']; ?>"><?php echo htmlspecialchars($u['name']) . " (" . $u['role'] . ")"; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
...
<script>
    ...
    const allUsers = <?php echo json_encode($all_users); ?>; // Pass all users for reports_to filtering
</script>