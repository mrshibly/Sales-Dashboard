...
// Permission check for GET (all roles can view analytics data)
if (!$auth->has_permission(['HOM', 'NSM', 'DSM', 'ASM', 'TSM', 'SR'])) {
    http_response_code(403);
    echo json_encode(['message' => 'Forbidden: You do not have permission to view analytics data.']);
    exit;
}
...
// Fetch sales data for a chart
$query_sales_data = "SELECT DATE_FORMAT(order_date, '%Y-%m') as month, SUM(oi.quantity * oi.price) as monthly_sales FROM orders o JOIN order_items oi ON o.id = oi.order_id JOIN customers c ON o.customer_id = c.id WHERE 1=1 " . $where_clause . " GROUP BY month ORDER BY month ASC";
$stmt_sales_data = $db->prepare($query_sales_data);
foreach ($bind_params as $key => $value) {
    $stmt_sales_data->bindValue($key, $value);
}
